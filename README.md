...
外部系统可以先用关键词搜索候选视频，再把用户选中的视频链接传给音频解析接口。音频解析接口返回完整 `downloadUrl`，外部系统可直接发起下载请求。
...
线上反向代理或预览环境会根据请求域名返回对应协议的完整 `downloadUrl`，例如 `https://example.com/api/stream/token`。
